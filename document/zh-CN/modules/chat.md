...
普通代码块右上方提供常驻「复制」按钮，只复制当前代码块中的代码文本，保留缩进与换行；流式输出期间可复制已经显示的内容。复制成功后按钮以原按钮颜色显示对勾和「已复制」，两秒后恢复；失败时在按钮内显示「复制失败」，不弹出顶部提示。
...
首页、会话页和项目面板使用同一套消息输入框。长文本会自动撑高输入框，达到最大高度后在框内滚动；Shift+Enter 换行、继续输入或粘贴文字时，光标所在行保持在底部工具栏上方可见。回到前文编辑时跟随当前光标，手动滚动阅读时保留滚动位置。

## 上传附件预览

文件上传完成后，点击输入框上方或已发送消息中的附件卡片，可在右侧 Canvas 打开预览；图片直接显示。Canvas 复用现有文档预览能力，不支持的格式会提示下载后打开。上传中或上传失败的文件不会打开预览。消息附件卡片上的下载图标及 Canvas 下载按钮仍可下载原文件。

## 会话内导航

会话页右侧的短线导航按已加载的每轮提问排列。悬停或用键盘聚焦可预览提问与回答摘要，点击可跳到同一会话的对应位置；当前阅读的轮次以深色长线标记。导航支持上下方向键、Home / End 切换焦点，Enter / Space 跳转，Esc 关闭预览。开启系统“减少动态效果”时直接定位。
...
发给模型的历史与界面展示分开存。助手消息除了 `content` / `thinking` / `tool_calls`
这三个展示列，还带一列 `model_steps`：按发生顺序记录每一次模型响应（思考、正文、
工具调用原样的块）和每一条进入上下文的工具结果。下一轮的模型上下文从这一列按序恢复，
思考与它触发的工具调用始终配在同一步里；用户中途停止时，尚未返回的调用会记一条
「已中断」的结果，回放时不会出现半步。写入该列之前的历史消息按 `metadata.segments`
记录的流式顺序重建；两者都没有的旧消息按一条带标注的历史摘要回放，不做猜测式配对。

上下文预算以「一步」为单位取舍：一步 = 模型的一次响应及其工具结果。思考只单独计数，
不会被单独截断或丢弃；预算不足时先裁较早步骤的工具输出，再从最旧一端整步切除，
本轮正在进行的步骤受保护，装不下时会在装配清单里标为 `over_budget` 而不是悄悄改写。

工具的模型结果步骤先独立落库，再处理展示事件，界面过滤不会丢弃模型历史。计划更新仍显示在计划栏，其真实工具结果保留供后续轮次回放；结果落库重试后仍失败时，本轮停止，不宣告工具完成。历史回放在对应调用后补齐缺失结果：优先使用已有记录，否则明确说明结果未记录、执行状态未知；不改写数据库历史。跨供应商或模型回放时，formatter 会核对思考的来源供应商、模型和协议；兼容思考保留原始结构及签名，不兼容或来源不明的可读思考转换为带明确标记的历史参考文本，不作为目标模型的原生推理状态。当前 Anthropic 模型的思考若缺失签名，会明确报错。
...
## 跨会话引用（Cross-chat References）

模型在一次对话里只看得见当前这段上下文。跨会话引用补上"翻旧账"那条路，分成一个能力和三个入口。

**能力：两个只读工具**。`list_related_chats` 列出可参考的历史会话（会话挂在项目下时只列同项目的，否则列本人最近的），返回标题、最后活跃时间和消息条数；`read_chat` 读某一段会话，`mode="full"` 按页取原文（单条与单次返回都有字数上限，用返回的 `next_offset` 翻页），`mode="digest"` 生成整段会话的概括，`mode="card"` 只要名片。作用域由注册时闭包里的 `user_id` 锁死，每次访问都过 `ChatService.get_session_with_access`——模型编一个 chat_id 也读不到别人的会话。

**入口一：模型自己找**。用户说"上次那个""我们之前讨论过的"时，模型先 `list_related_chats` 定位，再 `read_chat` 取内容。

**入口二：`/` 面板引用**。输入框打 `/` 后除技能与插件外多一组「引用会话」，继续打字按标题和消息正文检索（`GET /v1/chats/referencable`）。

**入口三：从侧边栏拖进输入框**。侧边栏会话条目在拖拽时额外携带 `application/x-hugagent-chat` 类型，输入框按类型分流：带这个类型的落点是"引用会话"，文件落点仍是"添加附件"，侧边栏内部的排序拖拽两者都不认。

**引用怎么进模型**：请求只上行被引用会话的 ID（`referenced_chats`），标题、时间、概览等名片信息一律由后端按当前用户权限现查，前端传什么都不作数。后端把名片渲染成一段文本拼在用户消息最前面（先于追问引用），并明确写着"细节请调 `read_chat` 取"。**注入的是名片不是全文**——一段长会话动辄几万字，直接拼进上下文会把当前话题挤掉。名片概览取材顺序是：已有的模型摘要缓存 → 上下文压缩检查点的摘要 → 首尾消息摘录，三条路都不调模型，所以引用不会拖慢发送。

名片随消息一起落库（`metadata.referenced_chats`），历史重放时按这份快照渲染，模型刷新前后看到的引用一字不差；重新生成与编辑重发同样沿用它。模型摘要只在 `read_chat(mode="digest")` 被显式调用时生成，生成后缓存在被引用会话上，直到该会话有新消息才重算。
...
原生看图工具返回的图片保持独立媒体块。主智能体和子智能体共用的工具结果截断只限制文字，图片不按 base64 字符数计入文字限额，也不会被写入文字 offload；统一上下文的文字裁剪同样保留图片。图片读取大小限制与整体上下文容量保护仍有效。

主对话中的子智能体调用行仅展示身份、状态和详情入口，不显示运行计时或完成耗时；右侧执行详情仍保留耗时信息。调用行沿用普通工具的箭头样式，箭头紧邻名称，仅在鼠标悬浮或键盘聚焦时显示。
...
显式选择技能、连接器和插件会加载对应能力，并通过提示词引导模型优先按需使用；不设置强制调用参数，也不因模型未调用工具而中止回答。使用技能时提示模型先读取对应 SKILL.md。所选能力仍须通过权限与可用性校验。加载成功后，后端以插件安装实例 ID 记录会话
激活态；后续回合会在默认能力筛选前恢复并重新校验它，因此工具名与 MCP 前缀保持稳定。插件被卸载、
管理员全局停用、依赖失效或归属变化时不会被恢复。

- **结构化 `@` 委派**：从输入框选择一个 `@子智能体` 时，前端同时提交
  `mention_agent_id` 和显示名。后端移除仅用于展示的 `@名称` 前缀，并向当前用户回合注入委派
  提示；主模型仍保留正常思考和逐 token 输出，提示模型优先按任务需要调用目标智能体的
  `call_subagent`。子智能体的完整执行均发生在该工具内部，其思考、工具和正文
...
- **自然语言显式委派**：以“调用”或“请调用”开头，并包含唯一、完整的可访问子智能体名称和
  明确动作任务时，后端只解析目标并向当前用户回合注入委派提示，不会伪造工具事件或绕过主模型。
  主模型保留正常思考和流式链路，提示模型优先按任务需要调用目标智能体的 `call_subagent`，也可使用其他工具辅助。
  ...
  或者“调用企业风险分析子智能体是否合适？”这类讨论句不会触发委派提示。
...
压缩只在合法边界切分：历史先按步骤展开，从尾部保留装得下
`CHAT_COMPACT_RECENT_USER_MAX_TOKENS` 的完整近期区域（切点只落在用户消息或某一步的
起点，且此前的工具调用都已返回），更早的区域交给摘要模型；替换历史 = 摘要 + 近期区域原样。
轮内压缩按消息内容块跟踪上下文变化，向已有 assistant 消息追加工具结果也会计入增量；
模型响应写入上下文后才记录本次 usage 基准，避免重复计算模型输出。历史被改写或替换时
重新估算，只有压缩后内容完全未变时才跳过重复压缩。
...
| 跨会话历史工具 | `src/backend/core/llm/tools/chat_history_tool.py` |
...
混合模式下，工具执行权限档以云端账户设置为准，选择后同时保存到本机执行端；
两端同步成功后才显示所选档位。打开桌面界面或本机执行端恢复就绪时，会检查并校准
本机设置。保存或读取失败会显示“权限未同步”，可重新选择档位重试。
权限档在新一轮任务装配时读取；已运行的任务和已有待确认操作不会因切换档位自动放行。

操作确认条统一显示“需要你确认一项操作”，并展示具体操作及路径或命令。
“本会话内同类操作均允许”仅授权当前确认所属的权限类别，不会放开其他类别的操作。


### 桌面插件的工具完整性

混合模式下，本机安装的插件说明与云端授权的工具可能来自不同安装。只有所需 MCP
均已进入本轮可用工具集合的插件才会出现在可加载目录中；缺少工具的插件不会以
“仅加载技能”的形式报告成功，其独有技能也不会作为可用能力展示。共享技能仍可由
另一份完整插件提供。显式调用不可用插件会报告缺少的工具，不会自动替换为其他来源。

混合模式不启动本机内置 MCP launcher。定时任务、批量执行及图表工具与其他
MCP 一样，使用当前账号授权的云端绑定，不再设按工具名称保留本机的例外。
旧的本机保留名单和整桥关闭环境变量已取消。用户主动配置的独立本机 MCP 仍可使用，
同名来源按显式选择及正常解析规则处理。

### 混合桌面渠道机器人的执行位置

在混合模式桌面端添加渠道机器人时，可以选择“云端”或“本机”，微信扫码绑定同样适用。
已有机器人默认在云端运行。选择本机需要本机执行服务就绪；机器人使用默认项目，
无需选择或绑定项目目录，按桌面现有工具权限访问本地内容。更改已有机器人的位置需先移除再添加。

本机机器人需要电脑在线、桌面端已登录且本机服务持续运行；最小化到托盘后服务仍运行即可继续。
列表分别显示渠道连接与本机在线状态。离线、中断或停用时不会自动改为云端执行；
执行结果不确定时不自动重试，请检查桌面记录后再重发。需要操作确认时会中止并提示，
不会自动放行；可在桌面处理权限后重新发起。

渠道接入、凭据和消息收发仍由云端负责。本机执行不等于完全离线，模型及已配置的云端工具
仍使用对应服务，回复和回传文件会经过云端及渠道平台。

### 子智能体执行详情

主对话中的子智能体调用行展示运行状态、智能体 Logo、名称与耗时。点击整行会在右侧 Canvas 打开本次调用的独立页签；内部过程不再在消息下方展开。运行状态动画与 Logo 分开显示。普通工具保持原来的折叠交互。

详情直接复用主对话的工具批次、思考和正文 UI，遵循相同的过程显示设置与默认折叠交互，不额外增加参数或执行过程分组。完成后调用行不显示对勾。面板实时跟随当前调用；向上滚动阅读时暂停自动跟随。同一智能体的多次调用分别定位，并可与文件、本体、插件页签共存。历史详情使用已保存的数据：子步骤最多 200 项，单条字符串工具输出约 16,000 字符，已截断内容无法恢复。

技能加载、插件加载以及可唯一确定所属连接器的工具调用行，展示能力中心中对应的 Logo。插件声明的工具专属图标优先使用。运行动画与 Logo 分开显示，点击行为和折叠布局保持不变；普通工具保留原分类图标。

Canvas 正文会去掉工具返回值中固定添加的「【智能体名称】的回复：」开头，保留智能体原文。工具调用展开条从首次显示起占满正文宽度，展开或收起具体工具时宽度保持一致。

连接器下的工具沿用连接器 Logo；插件内的技能和工具统一沿用插件 Logo。
