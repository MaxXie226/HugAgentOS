...
PyPI。完整运行时还携带 Pandoc 3.11 和 LibreOffice 26.2.6，供文档读取和 Office 转 PDF
使用；工具解压到应用私有目录，无需用户单独安装。联网只用于用户主动配置的在线模型、更新和联网工具。
构建时校验官方工具资产的大小和 SHA-256，构建及安装时执行真实 DOCX 读取和 PDF 转换自检。
因此完整包体积与解压空间会增加；精简包不包含这些工具。
...
如果一批安装包本来就只按「本机 + 云端」混合模式交付，可以在构建时关掉首启的运行模式选择：

```bash
JX_DEFAULT_SERVER_BASE=https://你的后端 npm run build:hybrid-only
```

这样打出的客户端首次启动不再询问运行模式和服务器地址，只展示带动画的初始化页和一个
「开始初始化」；确认后不重启应用，同一个窗口直接进入安装进度页并自动开始安装本机运行
环境。云端地址取构建时的 `JX_DEFAULT_SERVER_BASE`，缺失时构建直接失败。

不加这个选项时行为不变，首启仍是「仅本机 / 仅云端 / 本机 + 云端」三选一。

## 构建选项：本机端口命名空间

本机后端、脚本执行服务与内置 MCP 默认使用 `32101`、`8900`、`9100–9116`。同一台机器上安装两个
不同品牌的客户端时，用构建变量把其中一个整体挪开即可避免抢占端口：`JX_LOCAL_SERVER_PORT`、
`JX_LOCAL_SCRIPT_RUNNER_PORT`、`JX_LOCAL_MCP_PORT_OFFSET`。客户端会把后两者注入本机服务
进程，后端不再写死端口。升级换端口后，旧端口上的本机服务按记录的进程号与安装目录回收，
不会误伤同机其它产品。
