...
1. **先查目标**：调用 `list_sites()`，获取当前账号可修改站点的
   site_id、title、url、version、kind、source_dir、publish_dir、in_current_project。
   本机和云端都用这一个工具；**即使是新会话、即使是点「编辑」按钮进来的，也照样先查**。
   按用户提到的网址、标题及源码选择；`in_current_project=true` 的通常就是目标。
   多个候选无法区分时先询问，不能选最近发布的站点。工具报错或返回空不等于没有站点，
   要如实说明查询失败，不能自动新建，也不从目录名猜测编号。
   `editable=false` 是没有源码工程的老站点，改不了，只能征得用户同意后另建新站。
...
> `list_sites` 不可用时，不套用“后端按项目自动定位”的旧规则；读取最新技能并核对能力版本。

### 在全新对话里改老站点

用户新开一段对话说“帮我改一下之前那个站”时，不需要先切回原会话——`list_sites` 返回的
`source_dir` 当前就可直接读写（云端是项目文件夹、本机是绑定的本地目录），按这四步走即可：

1. `list_sites()` → 按用户说的网址或标题认出目标，多个拿不准就问；
2. 去该条的 `source_dir` 原地改（构建型在 `source_dir` 改完重新构建）；
3. `publish_site(title=…, site_id='<查到的编号>', src_dir='<静态站用 publish_dir／构建型用本次产物目录>')`；
4. 核对返回的 site_id 与 URL 没变、版本号 +1，再打开线上首页确认改动真的在，才交付。

**绝不能因为“这段对话没绑项目”就不带 site_id 直接发布**——那会新建一个站点，
用户原来的网址纹丝不动，看起来就是“改了但没更新”。
