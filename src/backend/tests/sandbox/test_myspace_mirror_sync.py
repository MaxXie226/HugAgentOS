"""镜像目录相对账本的判定与正向搬运，以及容器交接前清空 /workspace。
...
"什么时候登记"不在这里 —— 那由文件系统监听器决定，见 ``tests/myspace/``。
"""
...
def mirror(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "core.myspace.mirror._resolve_chain",
...
def test_a_materialized_file_does_not_look_like_a_sandbox_write(mirror, monkeypatch):
    """正向同步写下去的文件必须判成"已是最新"。

    判"磁盘比账本新 = 沙箱写的、还没登记"是全套机制的唯一依据。后端自己写镜像时若留下
    当下的 mtime，监听器就会把它当成沙箱的新改动转头再登记一次，用户还会莫名收到一次
    "是否允许覆盖"的确认 —— 所以写完要把 mtime 对齐登记时间。
    """
